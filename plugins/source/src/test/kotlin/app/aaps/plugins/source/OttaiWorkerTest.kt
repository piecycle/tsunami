...
class OttaiWorkerTest : TestBaseWithProfile() {

    private lateinit var worker: OttaiPlugin.OttaiWorker
    @Mock lateinit var ottaiPlugin: OttaiPlugin
    @Mock lateinit var persistenceLayer: PersistenceLayer
    @Mock lateinit var workerParameters: WorkerParameters

    init {
        addInjector { injector ->
            if (injector is OttaiPlugin.OttaiWorker) {
                injector.aapsLogger = aapsLogger
                injector.ottaiPlugin = ottaiPlugin
    ...
    @BeforeEach
    fun setupMock() {
        worker = OttaiPlugin.OttaiWorker(context, workerParameters)
    }

    @Test
    fun `When plugin disabled then return success`() {
        runBlocking {
            whenever(ottaiPlugin.isEnabled()).thenReturn(false)
    ...
    fun `When plugin enabled then insert data`() {
        ...
        runBlocking {
            whenever(ottaiPlugin.isEnabled()).thenReturn(true)
            ...
            verify(persistenceLayer).insertCgmSourceData(Sources.Ottai, listOf(expectedGv), emptyList(), null)
        }
    }
