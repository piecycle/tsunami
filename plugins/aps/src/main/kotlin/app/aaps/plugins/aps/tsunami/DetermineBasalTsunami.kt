...
@Singleton
class DetermineBasalTsunami @Inject constructor(
    ...
    fun enable_smb(profile: OapsProfileTsunami, microBolusAllowed: Boolean, meal_data: MealData, target_bg: Double): Boolean {
    ...
    private fun getMaxSafeBasal(profile: OapsProfileTsunami): Double =
    ...
    fun setTempBasal(_rate: Double, duration: Int, profile: OapsProfileTsunami, rT: RT, currenttemp: CurrentTemp): RT {
...
fun determine_basal(
        glucose_status: GlucoseStatus, currenttemp: CurrentTemp, iob_data_array: Array<IobTotal>, profile: OapsProfileTsunami, autosens_data: AutosensResult, meal_data: MealData,
        microBolusAllowed: Boolean, currentTime: Long, flatBGsDetected: Boolean, dynIsfMode: Boolean, recentGlucoseHistory: List<GlucoseStatus>, rawGlucoseHistory: List<GlucoseStatus>
): RT {
        consoleError.clear()
        consoleLog.clear()
        var rT = RT(
            algorithm = APSResult.Algorithm.TSUNAMI,
        ...
        //}
        //MP: The above code is redundant and thus removed from Tsunami (different to default openAPSSMB algorithm)
    ...
    // MP Early-stage decision between oref1 and tsunami loop algorithm (skip calculations if basic conditions aren't met)
        ...
        if (activityController) {
            ...
            */
            ...
            if (profile.insulinID != 105 && profile.insulinID != 205) {
                ...
                val td = profile.dia * 60 //MP Duration of insulin activity, in min
                val tau = tp * (1 - tp / td) / (1 - 2 * tp / td)
                val a = 2 * tau / td
                val S = 1 / (1 - a + (1 + a) * Math.exp((-td / tau)))
                tsuInsReq = actMissing / (S / Math.pow(tau, 2.0) * t * (1 - t / td) * Math.exp((-t / tau)))
            } else {
                // PD BASED MODEL CODE
                ...
                var tpModel: Double
                var actAtT = 0.000001 //MP Dummy value to enter while-loop
                var actRatio = 1.0 //MP initialising value
                tsuInsReq = 1.0 //MP Initial guess
        ...
        iobArray.forEach { iobTick ->
            ...
            // MP adjusted below for Tsunami. Default: 90; Here: profile.peakTime with no added 30m. This code was written for slower insulins than available today and recommended for use with Tsunami
        ...
        if (enableSMB && minGuardBG < threshold && !activityController) {
        ...
        //MP Skip the block below if activityController is active and it is triggered by BG predictions - they are mostly wrong during UAM and can result in persistent highs - other safety features are in place
        } else if (bg < threshold || (!activityController && minGuardBG < threshold)) {
        ...
        if (eventualBG < min_bg && !activityController) { // if eventual BG is below target:
        ...
        if (iob_data.iob > max_iob) {
            ...
            if (microBolusAllowed && enableSMB && bg > threshold) {
                ...
                // bolus 1/2 the insulinReq (oref1), up to maxBolus, rounding down to nearest bolus increment
                val roundSMBTo = 1 / profile.bolus_increment
                // MP Adjust SMB size by insulinReqPCT; Default value in oref1 is 0.5, Tsunami/Wave uses user-set values
                val microBolus = Math.floor(Math.min(insulinReq * insulinReqPCT, maxBolus) * roundSMBTo) / roundSMBTo
                // calculate a long enough zero temp to eventually correct back up to target
                ...
                if (activityController) {
                    ...
                    rT.reason.append("; act. now: " + profile.currentActivity)
                    rT.reason.append("; act. future: $actFuture")
                    rT.reason.append("; act. missing: $actMissing")
                    rT.reason.append("; ###")
                    rT.reason.append(" bg: $bg")
                    ...
                    rT.reason.append(" ##TSUNAMI STATUS END##")
                }
                ...
                if (lastBolusAge > SMBInterval - 6.0) {   // 6s tolerance
                    if (microBolus > 0) {
                        ...
                        insulinReq -= microBolus //MP Altered oref1 code: If SMB is issued, any TBR to follow needs to be lowered by the SMB.
                    }
                } else {
