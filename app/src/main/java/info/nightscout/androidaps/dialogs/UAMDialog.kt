...
//    @Inject lateinit var glucoseStatus: GlucoseStatus
    ...
    //MP test
    //var modestatus: Boolean = false
    //var modestatus = TempTarget().uam_switch_tt
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.plus05.text = sp.getDouble(resourceHelper.gs(R.string.key_UAM_button_increment_1), PLUS1_DEFAULT).toSignedString(activePlugin.activePump)
        binding.plus05.setOnClickListener {
            binding.amount.value = max(0.0, binding.amount.value
                    + sp.getDouble(resourceHelper.gs(R.string.key_UAM_button_increment_1), PLUS1_DEFAULT))
            validateInputs()
        }
        binding.plus10.text = sp.getDouble(resourceHelper.gs(R.string.key_UAM_button_increment_2), PLUS2_DEFAULT).toSignedString(activePlugin.activePump)
        binding.plus10.setOnClickListener {
            binding.amount.value = max(0.0, binding.amount.value
                    + sp.getDouble(resourceHelper.gs(R.string.key_UAM_button_increment_2), PLUS2_DEFAULT))
            validateInputs()
        }
    ...
    override fun submit(): Boolean {
        ...
        if (insulinAfterConstraints > 0 || eatingSoonChecked) {
            activity?.let { activity ->
                OKDialog.showConfirmation(activity, resourceHelper.gs(R.string.uam_mode), HtmlHelper.fromHtml(Joiner.on("<br/>").join(actions)), {//MP: String is header string of confirmation window if there is a prebolus
                    if (eatingSoonChecked) {
                    ...
                    if (insulinAfterConstraints > 0) {
                        ...
                        if (recordOnlyChecked) {
                            ...
                            aapsLogger.debug("USER ENTRY: PREBOLUS $insulinAfterConstraints")
