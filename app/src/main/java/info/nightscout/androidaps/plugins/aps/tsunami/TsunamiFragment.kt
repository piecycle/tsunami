...
class TsunamiFragment : DaggerFragment() {
    ...
    fun updateGUI() {
        ...
        val tsunamiPlugin = activePlugin.activeAPS
        tsunamiPlugin.lastAPSResult?.let { lastAPSResult ->
            binding.result.text = jsonFormatter.format(lastAPSResult.json)
            binding.request.text = lastAPSResult.toSpanned()
        }
        tsunamiPlugin.lastDetermineBasalAdapter?.let { determineBasalAdapterTAEJS ->
            binding.glucosestatus.text = jsonFormatter.format(determineBasalAdapterTAEJS.glucoseStatusParam)
            binding.currenttemp.text = jsonFormatter.format(determineBasalAdapterTAEJS.currentTempParam)
            try {
                val iobArray = JSONArray(determineBasalAdapterTAEJS.iobDataParam)
            ...
            binding.profile.text = jsonFormatter.format(determineBasalAdapterTAEJS.profileParam)
            binding.mealdata.text = jsonFormatter.format(determineBasalAdapterTAEJS.mealDataParam)
            binding.scriptdebugdata.text = determineBasalAdapterTAEJS.scriptDebug.replace("\\s+".toRegex(), " ")
            tsunamiPlugin.lastAPSResult?.inputConstraints?.let {
                binding.constraints.text = it.getReasons(aapsLogger)
            }
        }
        if (tsunamiPlugin.lastAPSRun != 0L) {
            binding.lastrun.text = dateUtil.dateAndTimeString(tsunamiPlugin.lastAPSRun)
        }
        tsunamiPlugin.lastAutosensResult.let {
