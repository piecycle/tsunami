 ...
 */
class GlucoseStatusTestSMB : TestBaseWithProfile() {
    ...
    @Test fun toStringShouldBeOverloaded() {
        val glucoseStatus = GlucoseStatusSMB(glucose = 0.0, noise = 0.0, delta = 0.0, shortAvgDelta = 0.0, longAvgDelta = 0.0, date = 0)
        assertThat(glucoseStatus.log(decimalFormatter)).contains("Delta")
    }

    @Test fun roundTest() {
        val glucoseStatus = GlucoseStatusSMB(glucose = 100.11111, noise = 0.0, delta = 0.0, shortAvgDelta = 0.0, longAvgDelta = 0.0, date = 0)
    ...
    @Test fun calculateValidGlucoseStatusAutoIsf() {
        ...
        val glucoseStatus = GlucoseStatusCalculatorSMB(aapsLogger, iobCobCalculator, dateUtil, decimalFormatter).getGlucoseStatusData(false)!!
    ...
    @Test fun returnOldDataIfAllowed() {
        Mockito.`when`(autosensDataStore.getBucketedDataTableCopy()).thenReturn(generateOldBgData())
        val glucoseStatus = GlucoseStatusCalculatorSMB(aapsLogger, iobCobCalculator, dateUtil, decimalFormatter).getGlucoseStatusData(true)
        assertThat(glucoseStatus).isNotNull()
    }

    @Test fun averageShouldNotFailOnEmptyArray() {
        assertThat(GlucoseStatusCalculatorSMB.average(ArrayList())).isWithin(0.001).of(0.0)
    }
